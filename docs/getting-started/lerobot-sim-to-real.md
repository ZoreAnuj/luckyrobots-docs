...
Train an imitation-learning policy on simulator recordings and deploy it to a real SO-100 arm — no manual teleoperation, no real-world fine-tuning required.
...
!!! success "72% success on the real SO-100"
    Checkpoint trained **only on simulator recordings.** No domain randomization. No real-data fine-tuning.
